 ...
 * to polling all fan
 */
static void npcm7xx_fan_polling(unsigned long arg)
{
	struct npcm7xx_pwm_fan_data *data = (struct npcm7xx_pwm_fan_data *)arg;
	unsigned long flags;
	int i;
...
static int npcm7xx_pwm_fan_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < NPCM7XX_FAN_MAX_CHN_NUM; i++) {
		if (data->fan_present[i] == true) {
			...
			data->npcm7xx_fan_timer.data = (ulong)data;
			data->npcm7xx_fan_timer.function = &npcm7xx_fan_polling;
			data->npcm7xx_fan_timer.expires = jiffies +
				msecs_to_jiffies(NPCM7XX_FAN_POLL_TIMER_200MS);

			init_timer(&data->npcm7xx_fan_timer);
			add_timer(&data->npcm7xx_fan_timer);
			break;
		}
	}
