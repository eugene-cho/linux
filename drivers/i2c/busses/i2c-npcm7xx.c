...
static inline void NPCM7XX_smb_nack(struct NPCM7XX_i2c *bus)
{
	if (bus->rd_ind < (bus->rd_size - 1))
		I2C_DEBUG("\tNACK error! bus%d, SA=0x%x, read (%d\%d), op=%d state=%d\n",
					  ...
					  bool use_PEC)
{
	...
	//
	// Configure FIFO mode only for master mode
	// Slave mode for linux will not use fifo
	...
	if (bus->fifo_use == true) {
		if (nread > 0) {
			...
			if (nread <= SMBUS_FIFO_SIZE) {
				// NPCM7XX_SMBRXF_CTL_THR_RXIE is set to zero.
				smbrxf_ctl = FIELD_PREP(NPCM7XX_SMBRXF_CTL_RX_THR, nread);
			}
			else {
				// if threshold_fifo != SMBUS_FIFO_SIZE set
				// NPCM7XX_SMBRXF_CTL.THR_RXIE 1 otherwise to 0
				smbrxf_ctl =
					FIELD_PREP(NPCM7XX_SMBRXF_CTL_RX_THR,
						bus->threshold_fifo) |
					FIELD_PREP(NPCM7XX_SMBRXF_CTL_THR_RXIE,
						(bool)(bus->threshold_fifo !=
						SMBUS_FIFO_SIZE));
			}
		...
		// TODO: replace with :	NPCM7XX_smb_set_fifo(bus, nread, -1);
...
static int NPCM7XX_smb_recovery(struct i2c_adapter *_adap)
{
	...
	if (FIELD_GET(NPCM7XX_SMBCST_TSDA , ioread8(NPCM7XX_SMBCST(bus))) == 0) {
		...
		do {
			...
			while ((FIELD_GET(NPCM7XX_SMBCST_TGSCL,
				ioread8(NPCM7XX_SMBCST(bus))) == 0) &&
				(--timeout != 0) );
			// If SDA line is inactive (high), stop
			if (FIELD_GET(NPCM7XX_SMBCST_TSDA,
					...
					struct i2c_msg *msgs, int num)
{
	...
	if (ret != -(EBUSY)){
		...
		if (time_left == 0 && bus->cmd_err == -EPERM){
			...
			NPCM7XX_smb_master_abort(bus);
			ret = -ETIMEDOUT;
			// TODO: add? complete(&bus->cmd_complete);
		}
		else
			ret = bus->cmd_err;
	}
